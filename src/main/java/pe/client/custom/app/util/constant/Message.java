package pe.client.custom.app.util.constant;

public class Message {
    public static final String MSG_EMAIL_WRONG_FORMAT = "El valor del email no es válido";
    public static final String MSG_PHONE_WRONG_FORMAT = "El valor del número de teléfono no es válido";
    public static final String MSG_PHONE_TYPE_WRONG_FORMAT = "El valor del tipo de número de teléfono no es válido";
    public static final String MSG_CURRENCY_WRONG = "El tipo de moneda no es válido";
    public static final String MSG_CARD_MONTH_WRONG = "El mes de expiracion de tarjera no es válido";
    public static final String MSG_CARD_YEAR_WRONG = "El año de expiracion de tarjera no es válido";
}
